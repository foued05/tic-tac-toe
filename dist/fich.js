...
function stateEmptySlot(plateau) {
    let slots = [];
...
function jeu(p, i) {
    let plateau_empty_slot = stateEmptySlot(plateau);
    let disp = '';
    plateau_empty_slot.map(ch => {
        if (ch == i.toString()) {
            const app = document.getElementById(p);
            disp = stateDisp();
            app === null || app === void 0 ? void 0 : app.setAttribute('src', disp);
